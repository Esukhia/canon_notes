...
def note_indexes(note):
    def side_indexes(note, extremity):
        ...
        #
        side = True
        while side and len(note_conc) == len([a for a in note_conc if note_conc[a] != []]):#len(note_conc[collection_eds[0]]) > 0:
...
def find_note_parts(note):
    ...
    for t in note:
        ...
        left_context = de_pre_process(left_context)
        right_context = note[t][right:]
        right_context = de_pre_process(right_context)
        notes[t] = (left_context, note_text, right_context)
    return notes
...
def generate_stats(data):
    ...
    write_profiles(profiles, dir='output/profiles')


def find_category(data, category, out_dir='output/note_categories'):
    # find all notes from a given category and write it to a csv file
    category_total = []
    for d in data[category]:
        note = copy.deepcopy(d)
        notes = find_note_parts(note)
        # only keep the syllables of the note text
        for n in notes:
            notes[n] = notes[n][1]
